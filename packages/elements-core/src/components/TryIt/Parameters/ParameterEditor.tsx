import { Flex, IInputProps, Input, Select, SelectProps, Text } from "@stoplight/mosaic";
...
export const ParameterEditor: React.FC<ParameterProps> = ({
  ...
  const inputType = parameter.schema?.type === 'number' ? 'number' : 'text'
  const inputOnChange: IInputProps['onChange'] = ({ currentTarget: { value } }) => onChange?.(inputType === "text" ? value : Number(value) )
