...
export const APIImpl: React.FC<APIProps> = props => {
  ...
  const [ prevDocument, setPrevDocument ] = useState<string>()
  ...
  useEffect(() => {
   if (fetchedDocument) {
     setPrevDocument(fetchedDocument)
   }
  }, [ fetchedDocument ])

  const document = apiDescriptionDocument || fetchedDocument || prevDocument || '';
